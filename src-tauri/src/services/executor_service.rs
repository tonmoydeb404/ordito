...
impl ExecutorService {
    ...
    pub async fn execute_command(&self, command: &Command, detached: bool) -> Result<Uuid> {
        info!("Executing command: {} (detached: {})", command.name, detached);
        ...
        if detached {
            ...
            tokio::spawn(async move {
                if let Err(e) = service_clone.run_command_internal(&command_clone, execution_handle).await {
    ...
    pub async fn execute_command_group(&self, commands: &[Command], detached: bool) -> Result<Vec<Uuid>> {
        info!("Executing command group with {} commands (detached: {})", commands.len(), detached);
    ...
    pub async fn get_execution_status(&self, execution_id: Uuid) -> Result<Option<CommandExecution>> {
    ...
    pub async fn kill_execution(&self, execution_id: Uuid) -> Result<()> {
        ...
        if let Some(execution_handle) = self.running_executions.lock().await.get(&execution_id) {
            ...
            if execution.is_running {
                execution.finish(Some(-1), String::new(), "Process killed by user".to_string());
                info!("Execution {} marked as killed", execution_id);
            }
            Ok(())
        } else {
            Err(OrditoError::Command(format!("Execution {} not found", execution_id)))
        }
    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        if running_count > 0 {
            ...
            for (id, execution_handle) in running.iter() {
                let mut execution = execution_handle.write().await;
                if execution.is_running {
                    execution.finish(Some(-1), String::new(), "Process terminated due to shutdown".to_string());
    ...
    async fn run_command_internal(&self, command: &Command, execution_handle: ExecutionHandle) -> Result<()> {
        ...
        match cmd.output().await {
            ...
            Err(e) => {
                ...
                Err(OrditoError::Command(format!("Failed to execute command: {}", e)))
            }
        }
    }
