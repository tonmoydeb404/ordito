...
pub fn parse_cron_expression(expression: &str) -> Result<Schedule> {
    Schedule::from_str(expression)
        .map_err(|e| OrditoError::CronExpression(format!("Invalid cron expression '{}': {}", expression, e)))
}
...
pub fn get_next_n_execution_times(expression: &str, count: usize) -> Result<Vec<DateTime<Utc>>> {
    let schedule = parse_cron_expression(expression)?;
    let upcoming: Vec<DateTime<Utc>> = schedule.upcoming(chrono_tz::UTC).take(count).map(|dt| dt.with_timezone(&Utc)).collect();
    
    if upcoming.is_empty() {
        return Err(OrditoError::CronExpression("No upcoming execution times found".to_string()));
    }
    ...
    match expression {
        ...
        _ => {
            if let Ok(schedule) = parse_cron_expression(expression) {
                ...
                let upcoming: Vec<DateTime<Utc>> = schedule.upcoming(chrono_tz::UTC).take(3).map(|dt| dt.with_timezone(&Utc)).collect();
                if !upcoming.is_empty() {
                    format!("Next runs: {}", 
                        upcoming.iter()
...
pub fn suggest_common_cron_patterns() -> Vec<(String, String)> {
    vec![
        ...
        ("0 0 * * 1".to_string(), "Weekly on Monday at midnight".to_string()),
        ("0 0 * * 0".to_string(), "Weekly on Sunday at midnight".to_string()),
        ("0 0 1 * *".to_string(), "Monthly on the 1st".to_string()),
        ("0 0 1,15 * *".to_string(), "Twice monthly (1st and 15th)".to_string()),
        ("0 0 1 1 *".to_string(), "Yearly on January 1st".to_string()),
        ("0 9 * * 1-5".to_string(), "Weekdays at 9 AM".to_string()),
        ("0 0 * * 6,0".to_string(), "Weekends at midnight".to_string()),
    ]
}

pub fn validate_and_describe_cron(expression: &str) -> Result<String> {
    if !is_cron_expression_valid(expression) {
        return Err(OrditoError::CronExpression(format!("Invalid cron expression: {}", expression)));
    }
...
pub fn time_until_next_execution(expression: &str) -> Result<chrono::Duration> {
    ...
    if next_time <= now {
        return Err(OrditoError::CronExpression("Next execution time is in the past".to_string()));
    }
...
mod tests {
    ...
    fn test_parse_valid_cron_expressions() {
        ...
        for expr in valid_expressions {
            assert!(parse_cron_expression(expr).is_ok(), "Expression '{}' should be valid", expr);
        }
    }
    ...
    fn test_parse_invalid_cron_expressions() {
        ...
        for expr in invalid_expressions {
            assert!(parse_cron_expression(expr).is_err(), "Expression '{}' should be invalid", expr);
        }
    }
    ...
    fn test_format_cron_description() {
        ...
        assert_eq!(format_cron_description("0 0 * * 0"), "Weekly on Sunday at midnight");
    }
    ...
    fn test_suggest_common_patterns() {
        ...
        for (expr, _desc) in patterns {
            assert!(is_cron_expression_valid(&expr), "Pattern '{}' should be valid", expr);
        }
    }
}